...
const Convert = ({ handleSwitch }) => {
  ...
  const bookingNotes =
    amountType.key === "usdt"
      ? buyingAmount / qmSellingPrice
      : amountType.key === "fiat"
      ? buyingAmount / usdtValue / qmSellingPrice
      : amountType.key === "qmgt"
      ? (buyingAmount * goldValue) / qmSellingPrice
      : 0;

  const recieveAmountUsdt = (goldValue * bookingNotes * 0.91).toFixed(2);
  const recieveAmountFiat = (
    goldValue *
    bookingNotes *
    0.91 *
    usdtValue
        ...
        <div className="w-full rounded-3xl bg-ash px-7 py-9  flex flex-col gap-3  shadow-2xl shadow-ash-light/40 border border-ash-light/40">
          ...
          <div className="flex items-center justify-between gap-3 w-full">
            ...
            <p className="text-smoke/80 text-base">
              {amountType.key === "fiat"
                ? `PHP ${recieveAmountFiat}`
                : `USDT ${recieveAmountUsdt}`}
            </p>
          </div>
          <div className="flex items-center justify-between gap-3 w-full">
